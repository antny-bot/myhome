export const copy = {
  ko: {
    // Navigation / Breadcrumb Parents
    home: "홈",
    alert: "알림",
    search: "조회",
    manager: "관리자",
    setting: "설정",

    // Common / Breadcrumbs
    breadcrumb: "실거래가",
    subBreadcrumb: "아파트 매매",
    title: "실거래 집계 현황",
    subtitle: "등록된 지역별 실거래 통계를 확인 및 관리하며, 매일 새벽 6시 이후 업데이트 됩니다.",
    addRegionTitle: "신규 수집 지역 등록",
    addRegionSuccess: "지역이 수집 대상으로 추가되었습니다. (매일 새벽 자동 수집)",
    deleteRegionSuccess: "지역이 수집 대상에서 삭제되었습니다.",
    regionsListTitle: "수집 및 집계 지역 목록",
    dbAggregatePeriod: "집계 기간",
    dbCollectCount: "수집 건수",
    registeredAt: "등록일",
    noDbRegions: "등록된 수집 지역이 없습니다. 새로운 지역을 검색하여 추가해 보세요.",
    viewDetails: "조회",
    deleteRegion: "삭제",
    tradeType: "거래 유형",
    sale: "매매",
    jeonse: "전세",
    monthlyRent: "월세",
    notReady: "준비 중인 기능입니다",
    region: "지역 (시군구)",
    period: "거래년월 / 기간",
    rangeSearch: "기간 조회",
    startMonth: "시작 YYYYMM",
    endMonth: "종료 YYYYMM",
    singleMonth: "YYYYMM",
    searchButton: "조회하기",
    complexFilter: "단지명",
    areaMin: "전용 최소(㎡)",
    areaMax: "전용 최대(㎡)",
    priceMin: "가격 최소(억)",
    priceMax: "가격 최대(억)",
    resetFilters: "필터 초기화",
    recordCount: "거래 건수",
    averagePrice: "평균 거래가",
    averagePpy: "평균 평단가",
    priceRange: "최고 · 최저",
    searchScope: "조회 기간 전체",
    ppyHint: "전용면적 기준 환산",
    resultTitle: "실거래 목록",
    resultSubtitle: "조건별 필터링, 주요 통계(KPI) 및 단지별 시계열/표 형태의 결과를 조회합니다.",
    groupedView: "단지별 시계열",
    tableView: "표",
    latestSort: "최신 거래일순",
    highPriceSort: "가격 높은순",
    lowPriceSort: "가격 낮은순",
    largeAreaSort: "면적 넓은순",
    highPpySort: "평단가 높은순",
    showing: "조회 결과",
    allResults: "전체",
    filteredResults: "필터 적용",
    apartmentName: "아파트명",
    area: "전용면적",
    floor: "층",
    price: "거래금액",
    ppy: "평단가",
    dealDate: "거래일",
    delta: "직전 대비",
    noChange: "변동 없음",
    loading: "조회 중...",
    noResults: "조건에 맞는 실거래 내역이 없습니다.",
    prompt: "지역과 기간을 선택해 조회하세요.",
    selectRegionFirst: "지역을 먼저 검색해 선택해 주세요.",
    monthFormatError: "기간은 YYYYMM 형식으로 입력해 주세요.",
    searchFailed: "실거래 조회에 실패했습니다.",
    regionPlaceholder: "지역 검색 이동 : (예) 분당구, 강남구",
    sourceNote: "",
    unitDeal: "억",
    unitCount: "건",
    unitPpy: "만원/평",
    allTradeTypeNote: "현재는 매매만 조회할 수 있습니다.",
    mapView: "지도 보기",
    listView: "목록 보기",
    loadingMap: "지도를 불러오는 중...",
    mapLoadError: "지도 로드에 실패했습니다. API 키를 확인해 주세요.",
    avgPriceLabel: "평균",
    maxPriceLabel: "최고",
    tradeCount: "거래",

    // Navigation Groups
    groupHome: "홈",
    groupExplore: "조회",
    groupAlert: "알림",
    groupAdmin: "관리자",
    groupSettings: "설정",

    // Dashboard
    dashboardTitle: "실거래 집계 현황",
    dashboardSubtitle: "관심 지역의 실거래 및 알림 발송 현황을 확인하세요.",
    bottomSheetTitle: "주요 지표 및 활동 이력",
    bottomSheetExpandHint: "위로 슬라이드하거나 터치하여 상세 보기",
    bottomSheetCollapseHint: "터치하여 접기",
    activeRules: "활성 조건",
    totalMatches: "누적 매칭",
    sentNotifications: "발송 알림",
    systemStatus: "상태",
    statusNormal: "정상",
    statusCheck: "점검",
    alertHistory: "알림 발송 이력",
    noAlertHistory: "알림 발송 이력이 없습니다.",
    alertSuccess: "성공",
    alertSkipped: "제외",
    alertFailed: "실패",
    alertPrefix: "알림",
    dbStatsTitle: "실거래 DB 현황",
    dbStatsRegionCount: "수집 지역",
    dbStatsComplexCount: "등록 단지",
    dbStatsDealCount: "누적 실거래",
    dbStatsSubtitle: "로컬 SQLite 적재 및 수집 현황",
    accordionCollapse: "접기",
    accordionExpand: "펼치기",
    accordionHistoryCollapsed: "이력이 접혀 있습니다. 클릭하여 실행 결과를 확인하세요.",
    accordionAlertHistoryCollapsed: "알림 이력이 접혀 있습니다. 클릭하여 발송 이력을 확인하세요.",

    // Rules
    rulesTitle: "알림 규칙",
    rulesSubtitle: "관심 지역의 실거래를 자동으로 추적할 조건을 만들고 관리하세요.",
    ruleFormCreate: "관심 조건 만들기",
    ruleFormEdit: "관심 조건 수정",
    ruleName: "조건 이름",
    ruleNamePlaceholder: "예: 우리 집 주변 급매 체크",
    regionSearch: "지역명 검색",
    aptSelect: "아파트 단지 선택",
    aptSelectDesc: "알림을 받고 싶은 단지들을 선택해 주세요 (미선택 시 전체 알림)",
    aptSearchPlaceholder: "단지명을 입력해 검색 및 추가하세요...",
    aptSearchInputPlaceholder: "단지명 입력 (미입력 시 전체 조회)",
    priceRangeLabel: "가격 범위 (억)",
    priceMinPlaceholder: "최소",
    priceMaxPlaceholder: "최대",
    areaRangeLabel: "전용면적 범위 (㎡)",
    areaMinPlaceholder: "최소",
    areaMaxPlaceholder: "최대",
    comparisonCriteria: "비교 기준",
    comparisonCriteriaNone: "가격 조건만",
    comparisonCriteriaParking: "주차 우선",
    comparisonCriteriaLargeComplex: "대단지 우선",
    comparisonCriteriaTransit: "교통 우선",
    comparisonCriteriaNewer: "신축/연식 우선",
    comparisonCriteriaLivability: "생활편의성 우선",
    ruleInitialName: "관심 지역 실거래가 체크",
    checkInterval: "알림 시간",
    checkIntervalNote: "일단위로 지정된 시간에 최신 실거래 내역을 체크하여 알림을 보냅니다.",
    alertChannels: "알림 채널",
    channelTelegram: "텔레그램 (기본)",
    channelKakao: "카카오 나에게 보내기 (예약)",
    buttonPrev: "이전",
    buttonNext: "다음 단계",
    buttonSave: "조건 저장 및 활성화",
    buttonSaveProgress: "저장 중",
    buttonSaveComplete: "수정 완료",
    ruleListTitle: "관심 조건 목록",
    noRules: "저장된 관심 조건이 없습니다. 신규 조건을 추가해 보세요.",
    ruleSuspended: "중단됨",
    ruleAllComplexes: "전체 단지",
    ruleRecentTrack: "최근 2개월 자동 추적",
    rulePriceRange: "억~",
    ruleUnlimited: "무제한",
    ruleIntervalSuffix: "분 주기",
    ruleLastChecked: "체크",
    ruleEdit: "수정",
    ruleDelete: "삭제",
    rulePause: "일시정지",
    ruleResume: "활성화",
    ruleRunNow: "지금 실행",
    ruleRunProgress: "진행 중",
    deleteConfirm: "정말 삭제하시겠습니까?",

    // Analytics
    analyticsTitle: "실거래 분석 리포트",
    analyticsSubtitle: "수집된 아파트 실거래 데이터의 시계열 흐름과 다차원 통계를 분석합니다.",
    analyticsTabOverview: "종합 현황",
    analyticsTabComplex: "단지 분석",
    analyticsTabInsight: "AI 인사이트",
    analyticsTabNearby: "역세권 분석",
    totalTradeVolume: "총 거래량",
    averageTradePrice: "평균 거래가",
    highestTrade: "최고가 거래",
    lowestTrade: "최저가 거래",
    nearbyRadiusLabel: "검색 반경",
    enableLiveLabel: "실시간 국토부 API 연동 조회",
    liveSearchDesc: "국토부 API 실시간 호출로 검색 시간이 5초 이상 소요될 수 있으며, 최근 3개월 거래 단지만 표시됩니다.",
    nearbyRadius300m: "300m (도보 5분)",
    nearbyRadius500m: "500m (도보 7분)",
    nearbyRadius700m: "700m (도보 10분)",
    nearbyRadius1000m: "1km (도보 15분)",
    nearbyRadius1500m: "1.5km (도보 20분)",
    nearbyRadius2000m: "2km (도보 30분)",
    boxPlotTitleArea: "평형별 거래가 분포 & 거래량",
    boxPlotTitleFloor: "층별 거래가 분포 & 거래량",
    boxPlotTitleRegion: "주요 지역별 거래가 분포 & 거래량",
    boxPlotTitleUnion: "실거래가 분포 & 거래량",
    dimensionArea: "평형별",
    dimensionFloor: "층별",
    pivotTableTitle: "지역별·평형대별 실거래 요약",
    pivotTableLegend: "* 셀에 마우스를 올리면 상세 실거래 정보를 확인할 수 있습니다.",
    xaxisPyeong: "평형 (평)",
    xaxisM2: "전용면적 (㎡)",
    boxPlotWhisker: "최소~최대 범위 (Whisker)",
    boxPlotBox: "Q1 ~ Q3 분포 박스",
    boxPlotMedian: "중위값 (Median)",
    boxPlotAvg: "평균가 (Avg)",
    boxPlotVolume: "거래량 (우측, 배경)",
    floorLow: "저층 (5층 이하)",
    floorMid: "중층 (6~15층)",
    floorHigh: "고층 (16~25층)",
    floorSuper: "초고층 (26층 이상)",
    allRegions: "전체 지역",
    complexActiveRankTitle: "거래가 활발한 아파트 단지",
    complexActiveRankTitleWithLimit: "거래가 활발한 아파트 단지 (상위 {limit}개)",
    complexClickHint: "* 단지명(X축) 또는 그래프를 클릭하면 상세 실거래 추이를 조회할 수 있습니다.",
    showLegend: "범례 표시",
    hideLegend: "범례 숨기기",
    legendVolume: "거래 건수 (Bar)",
    legendAvgPrice: "평균 거래금액 (Line)",
    noActiveComplexData: "해당 지역에 활성화된 거래 정보가 없습니다.",
    noActiveChartData: "표시할 데이터가 없습니다. 범례에서 그래프를 활성화해주세요.",
    axisVolume: "거래수 (건)",
    axisAvgPrice: "평균가 (억)",

    // Collect Stats
    collectTitle: "수집 데이터 리포트",
    collectSubtitle: "수집봇을 통해 DB에 적재된 일단위 및 월별 실거래 수집 통계를 분석합니다.",
    collectReportTitle: "수집 일단위 리포트",
    collectReportSubtitle: "수집봇을 통해 DB에 적재된 일단위 실거래 수집 건수를 분석합니다.",

    // Activity Log
    dailyCollectChart: "일자별 수집 건수",
    collectCount: "수집 건수",
    selectDatePrompt: "그래프에서 날짜를 클릭하면 상세 지역별 수집 건수가 여기에 표시됩니다.",
    selectedDateStats: "수집 지역별 통계",
    regionName: "지역명",
    lawdCode: "지역 코드",
    accumulatedCollectCount: "누적 수집 건수",
    totalAvgPrice: "전체 평균 거래가",
    maxComplexCount: "최대 단지 수",
    unitComplex: "개",
    unitDealWon: "억 원",
    periodRecent30: "최근 30일",
    periodAll: "전체",
    dailyView: "일자별 (수집일 기준)",
    monthlyView: "등록월별 (계약일 기준)",
    monthlyCollectChart: "등록월별 수집 건수",
    selectMonthPrompt: "그래프에서 등록월을 클릭하면 상세 지역별 수집 건수가 여기에 표시됩니다.",
    selectedMonthStats: "등록월 지역별 통계",
    dealMonth: "등록월",

    // Database Admin
    dbAdminTitle: "데이터베이스 관리자",
    dbAdminSubtitle: "SQLite 데이터베이스 스키마를 조회하고, 직접 SQL 쿼리를 실행하여 조회 및 수정을 제어합니다.",
    execute: "쿼리 실행",
    sqlPlaceholder: "여기에 SQL 쿼리를 입력해 주세요. (예: SELECT * FROM transactions LIMIT 10;)",
    queryResult: "실행 결과",
    tablesList: "테이블 목록",
    schemaInfo: "컬럼 정보",
    querySuccess: "쿼리가 성공적으로 실행되었습니다.",
    rowsCount: "행",
    affectedRows: "영향받은 행 수",
    lastInsertId: "마지막 삽입 ID",
    sqlTemplate: "SQL 템플릿",
    dbTabSql: "SQL 탐색기",
    dbTabGeocoding: "위치/좌표 관리",
    dbTabManagement: "데이터 관리",

    // Settings
    settingsTitle: "환경 설정",
    settingsSubtitle: "데이터 소스, 외부 API 키 및 실시간 알림 채널 연동 상태를 관리하세요.",
    systemStatusTitle: "시스템 연동 상태",
    settingsSaved: "시스템 설정이 성공적으로 저장 및 적용되었습니다.",
    settingsSaveFailed: "설정 저장 실패",

    // Allowed Accounts
    allowedAccounts: "계정 관리",
    allowedAccountsTitle: "계정 관리",
    allowedAccountsSubtitle: "대시보드에 접근할 수 있는 계정을 등록하고 임시 비밀번호를 관리합니다.",
    allowedAccountsListTitle: "등록된 계정 목록",
    addAccount: "계정 추가",
    emailPlaceholder: "이메일 주소 입력 (예: user@gmail.com)",
    deleteAccountConfirm: "이 계정을 목록에서 삭제하시겠습니까?",
    addSuccess: "계정이 성공적으로 추가되었습니다.",
    deleteSuccess: "계정이 성공적으로 삭제되었습니다.",
    invalidEmail: "올바른 이메일 형식이 아닙니다.",
    alreadyExists: "이미 등록된 이메일입니다.",
    noAllowedAccounts: "등록된 계정이 없습니다. 모든 사용자의 로그인이 제한될 수 있습니다.",
    role: "권한",
    userRoleNormal: "일반 사용자",
    userRoleAdmin: "관리자",
    toggleAdminSuccess: "권한이 성공적으로 변경되었습니다.",
    cantRemoveSelfAdmin: "본인의 관리자 권한은 해제할 수 없습니다.",
    atLeastOneAdmin: "최소 한 명 이상의 관리자가 존재해야 합니다.",
    resetPasswordTooltip: "임시 비밀번호 재발급",
    resetPasswordConfirm: "{email} 계정의 비밀번호를 초기화하고 임시 비밀번호를 재발급하시겠습니까?",
    resetPasswordSuccess: "비밀번호가 성공적으로 초기화 및 재발급되었습니다.",
    tempPasswordIssued: "임시 비밀번호가 발급되었습니다.",
    tempPasswordAlert: "임시 비밀번호가 생성되었습니다. 이 창을 닫으면 다시 확인할 수 없으니 꼭 복사해 두세요.",
    copyTempPassword: "임시 비밀번호 복사",
    copySuccess: "클립보드에 복사되었습니다!",
    isAdminUser: "관리자 권한 부여",
    changePasswordTitle: "비밀번호 변경 필요",
    changePasswordSubtitle: "임시 비밀번호로 로그인하셨습니다. 안전한 서비스 이용을 위해 비밀번호를 변경해 주세요.",
    newPassword: "새 비밀번호",
    confirmNewPassword: "새 비밀번호 확인",
    passwordMismatch: "비밀번호가 일치하지 않습니다.",
    passwordMinLength: "비밀번호는 8자 이상이어야 합니다.",
    changePasswordSuccess: "비밀번호가 성공적으로 변경되었습니다. 이제 대시보드를 이용하실 수 있습니다.",
    changePasswordFailed: "비밀번호 변경에 실패했습니다.",

    // Nearby Station Search
    nearbyStationTitle: "역세권 아파트 검색",
    nearbyStationSubtitle: "지하철역 주변 관심 반경 내의 아파트 단지 정보와 위치를 검색합니다.",
    stationPlaceholder: "예: 판교역, 강남역",
    nearbyComplexesTitle: "반경 내 단지 정보",
    clickComplexToAnalyze: "단지 위치를 클릭하면 단지 상세 분석으로 이동합니다.",

    // Region Map Explorer (지역별 단지 지도)
    regionMapTitle: "지역별 단지 지도",
    regionMapSubtitle: "수집된 지역구 내 아파트 단지 위치와 최근 실거래가를 지도에서 한눈에 확인하고 단지 분석으로 연결합니다.",
    regionMapNavLabel: "단지 지도",
    loadedRegions: "적재 지역",
    selectRegionToViewMap: "지도를 확인할 지역구를 선택하세요.",
    totalComplexesCount: "총 {count}개 단지",
    geocodedComplexesCount: "좌표 등록 {count}개",
    noComplexesInRegion: "선택한 지역에 적재된 아파트 단지가 없습니다.",
    searchComplexInRegion: "단지명 또는 동 검색...",
    allDongs: "전체 동",
    latestDealPrice: "최근 실거래가",
    avgDealPrice: "평균 거래가",
    dealCountLabel: "거래 건수",
    viewComplexAnalysisBtn: "단지 분석 바로가기",
    addRuleForComplex: "알림 규칙 추가",
    sortByTxCount: "거래 많은순",
    sortByLatestDeal: "최근 거래순",
    sortByPriceHigh: "높은 가격순",
    sortByPriceLow: "낮은 가격순",
    sortByName: "가나다순",
    paginationInfo: "총 {total}개 중 {from}~{to}개",
    pageItemsCount: "{count}개씩",
    firstPageBtn: "처음 페이지",
    prevPageBtn: "이전 페이지",
    nextPageBtn: "다음 페이지",
    lastPageBtn: "마지막 페이지",
    autoGeocodeNotice: "좌표가 등록되지 않은 단지가 {count}개 있습니다.",
    runBatchGeocodeForRegion: "단지 좌표 일괄 수집",
    mapViewToggle: "지도 보기",
    listViewToggle: "목록 보기",
    viewOnMap: "지도 보기",
    showOnMap: "지도에서 위치 확인",
    centerToStation: "지하철역 중심으로",
    geocodeStatsBtn: "좌표 캐싱 설정",
    geocodeStatsTitle: "Geocoding 좌표 캐싱 관리",
    geocodePendingListTitle: "위경도 좌표가 없는 단지 목록",
    noGeocodePendingComplexes: "위경도 좌표가 없는 단지가 없습니다.",
    noNearbyComplexes: "반경 이내에 등록된 아파트 단지가 없습니다.",
    geocodePercentageOfTotal: "좌표 데이터 현황",
    geocodeRequiredDesc: "현재 DB에 등록된 아파트 중 {pending}개 단지의 위도·경도 좌표가 없습니다. 검색 속도 향상을 위해 국토부 지번 주소 기반으로 카카오 Geocoding 일괄 수집을 실행할 수 있습니다.",
    runGeocodeBatchBtn: "좌표 미확보 단지 일괄 수집",
    batchGeocodeCompleted: "Geocoding 수집 배치 완료",
    batchGeocodeCountInfo: "대상: {total}건 / 성공: {success}건 / 실패: {failed}건",
    dbAggregateTab: "사전 집계 기반",
    liveSearchTab: "실시간 조회 기반",
    allComplexAlertRegister: "전체 단지 알림 등록",
    alertRegisterTip: "반경 내 모든 단지를 관심 조건 알림 규칙으로 등록합니다.",
    complexAnalysisLink: "단지 분석으로 이동",
    alertRegisterBtn: "알림 규칙 등록",
    stationSearchFailed: "지하철역 조회에 실패했습니다.",
    geocodeBatchFailed: "Geocoding 배치 실행에 실패했습니다.",
    geocodeBatchPausing: "수집 대기 중 (API 제한 방지)...",
    geocodeCollecting: "수집 중",
    distance: "거리",
    telegramGuideTitle: "텔레그램 알림 설정 가이드",
    telegramGuideSubtitle: "아파트 실거래 조건 충족 시 알림을 실시간 수신할 텔레그램 설정을 안내합니다.",
    telegramStep1: "1단계: 봇 생성하기 (BotFather)",
    telegramStep1Desc: "Telegram 앱에서 @BotFather를 찾아 대화를 시작한 후 /newbot 명령을 전송하여 봇을 만듭니다. 생성 과정 중 봇의 이름과 Username을 지정하면, 고유한 'HTTP API Token'을 획득할 수 있습니다.",
    telegramStep2: "2단계: 봇과의 개인 대화방 생성 및 /start 전송",
    telegramStep2Desc: "생성된 봇의 Telegram 대화 링크를 클릭하여 대화를 시작하고 반드시 /start 메시지를 보내야 봇이 메시지를 발송할 수 있는 권한이 활성화됩니다.",
    telegramStep3: "3단계: 대화방 ID (Chat ID) 알아내기",
    telegramStep3Desc: "수신할 대화방 ID를 찾기 위해 Telegram에서 @getidsbot을 검색해 대화방에 들어가거나, 봇을 초대한 그룹방에 해당 ID 봇을 초대하여 Chat ID를 얻습니다. (개인 ID는 9~10자리 양수, 그룹/채널 ID는 -로 시작하는 음수입니다.)",
    telegramStep4: "4단계: 환경설정 저장",
    telegramStep4Desc: "대시보드의 '환경 설정 > 외부 API 연동 설정 > 텔레그램 알림 채널' 메뉴로 이동하여 획득한 '봇 토큰'과 '대화방 ID'를 입력한 후 저장합니다.",
    telegramOpenBotFather: "@BotFather 바로가기",
    telegramOpenGetIdsBot: "@getidsbot 바로가기",
    telegramCopy: "복사",
    telegramCopied: "복사 완료!",
    telegramClose: "닫기",
    activityLog: "활동 로그",
    activityLogTitle: "활동 로그 모니터링",
    activityLogSubtitle: "대시보드 내 사용자의 페이지 방문, 실거래 검색, 수집 지역 변경 등의 활동 로그를 모니터링합니다.",
    activityType: "활동 유형",
    activityDescription: "활동 내용",
    ipAddress: "IP 주소",
    userAgent: "User Agent",
    time: "시간",
    filterUser: "사용자 이메일 필터",
    filterType: "활동 유형 필터",
    allTypes: "전체 유형",
    noActivityLogs: "기록된 활동 로그가 없습니다.",
    activityTrends: "최근 활동 추이 (14일)",
    activityVolume: "활동량 (로그 수)",
    activeUsers: "액티브 사용자 (명)",
    activityDistribution: "활동 유형별 분포",
    payload: "추가 정보",
    viewPayload: "상세 데이터",
    recentSearch: "최근 검색",
    recentSearchClear: "전체 삭제",
    recentRegions: "최근 검색 지역",
    recentStations: "최근 검색 지하철역",
    filterDate: "일자 필터",
    allDates: "전체 일자",
    clearDateFilter: "일자 필터 초기화",
    activityChartClickHint: "그래프의 일자를 클릭하면 해당 일자의 상세 활동 로그만 필터링됩니다.",
    filteredByDate: "{date} 활동 로그",
    activityType_page_view: "페이지 뷰",
    activityType_search_transactions: "실거래 검색",
    activityType_region_add: "지역 추가",
    activityType_region_delete: "지역 삭제",
    activityType_rule_create: "규칙 생성",
    activityType_rule_update: "규칙 수정",
    activityType_rule_delete: "규칙 삭제",
    activityType_rule_test: "규칙 테스트",
    activityType_preset_create: "프리셋 추가",
    activityType_preset_delete: "프리셋 삭제",
    
    // 입지 분석 평점
    infraRatingTitle: "⭐ 단지 주변 입지 평가 리포트",
    infraRatingSubtitle: "반경 1km 이내의 핵심 생활 인프라 시설의 최단 거리를 가중치 분석하여 산출한 종합 평가 점수입니다.",
    infraTotalScore: "종합 평가 점수",
    infraGrade: "입지 등급",
    infraCategory: "인프라 부문",
    infraScore: "부문 점수",
    infraCount: "시설 개수",
    infraMinDistance: "최단 거리",
    infraWalkMin: "도보 약 {min}분",
    infraMockNotice: "⚠️ 카카오 API 키가 설정되지 않아 샘플 입지 데이터로 표시 중입니다.",
    infraCategorySchool: "학교",
    infraCategoryHospital: "병원",
    infraCategoryMart: "대형마트",
    infraCategoryPharmacy: "약국",
    infraCategoryConvStore: "편의점",
    infraDetailGeneralHospital: "종합/대학병원",
    infraDetailLocalClinic: "일반 병/의원",
    infraDetailLargeMart: "대형마트",
    infraDetailSSM: "대형슈퍼(SSM)",
    infraDetailElementary: "초등학교",
    infraDetailMiddle: "중학교",
    infraDetailHigh: "고등학교",
    infraDetailMetro: "지하철(일반)",
    infraDetailGtx: "GTX",
    infraDetailRail: "광역/고속철도(기차)",
    infraDetailPharmacy: "약국",
    infraDetailConvStore: "편의점",
    mapGuideTooltip: "지도에서 지역구를 클릭하거나 검색하여 실거래 수집/집계 기간을 관리하세요.",
    mapCollectPeriod: "집계 기간",
    mapTransactionCount: "수집 건수",
    mapCollectPromptTitle: "실거래 집계 설정",
    mapCollectPromptNewRegion: "신규 지역 실거래 집계",
    mapCollectPromptExistingRegion: "실거래 추가 집계 (기간 연장)",
    mapCollectPromptConfirm: "집계 시작",
    mapCollectPromptCancel: "취소",
    mapCollectSuccess: "성공적으로 실거래 데이터가 집계되었습니다.",
    mapCollectFailed: "실거래 수집에 실패했습니다.",
    mapCollecting: "국토부 실거래 수집 중...",
    collectProgressTitle: "실거래 데이터 수집 진행 상황",
    collectProgressStatus: "집계 중",
    collectingProgressText: "실거래 수집 중",
    stopBtn: "중단",
    progressStatsTitle: "진행 수치",
    collectFailuresTitle: "실거래 수집 실패 목록",
    monthLabel: "대상월",
    reasonLabel: "실패 사유"
  },
  en: {
    // Navigation / Breadcrumb Parents
    home: "Home",
    alert: "Alerts",
    search: "Search",
    manager: "Admin",
    setting: "Settings",

    // Navigation Groups
    groupHome: "Home",
    groupExplore: "Explore",
    groupAlert: "Alerts",
    groupAdmin: "Admin",
    groupSettings: "Settings",

    // Common / Breadcrumbs
    breadcrumb: "Transactions",
    subBreadcrumb: "Apartment Sales",
    title: "Transaction Explorer",
    subtitle: "View and manage transaction stats for registered regions (automatically updated daily at 6 AM via MOLIT API).",
    addRegionTitle: "Register New Region",
    addRegionSuccess: "Region added to collection targets (collected automatically every morning).",
    deleteRegionSuccess: "Region removed from collection targets.",
    regionsListTitle: "Collection & Aggregation Regions",
    deleteRegion: "Delete",
    tradeType: "Trade Type",
    sale: "Sale",
    jeonse: "Jeonse",
    monthlyRent: "Monthly Rent",
    notReady: "Coming soon",
    region: "Region (City/District)",
    period: "Deal Month / Range",
    rangeSearch: "Range search",
    startMonth: "Start YYYYMM",
    endMonth: "End YYYYMM",
    singleMonth: "YYYYMM",
    searchButton: "Search",
    complexFilter: "Apartment",
    areaMin: "Min area (㎡)",
    areaMax: "Max area (㎡)",
    priceMin: "Min price (Eok)",
    priceMax: "Max price (Eok)",
    resetFilters: "Reset filters",
    recordCount: "Transactions",
    averagePrice: "Average price",
    averagePpy: "Average price / pyeong",
    priceRange: "High · Low",
    searchScope: "Across the selected period",
    ppyHint: "Calculated from exclusive area",
    resultTitle: "Transaction Table",
    resultSubtitle: "View transaction lists with filters, KPI stats, and toggle between complex group series or table views.",
    groupedView: "Complex Series",
    tableView: "Table",
    latestSort: "Latest deal date",
    highPriceSort: "Highest price",
    lowPriceSort: "Lowest price",
    largeAreaSort: "Largest area",
    highPpySort: "Highest pyeong price",
    showing: "Showing",
    allResults: "All",
    filteredResults: "Filtered",
    apartmentName: "Apartment",
    area: "Area",
    floor: "Floor",
    price: "Price",
    ppy: "Price / pyeong",
    dealDate: "Deal date",
    delta: "Prev delta",
    noChange: "No change",
    loading: "Loading...",
    noResults: "No transactions matched the current filters.",
    prompt: "Choose a region and time range to begin.",
    selectRegionFirst: "Search and select a region first.",
    monthFormatError: "Use YYYYMM format for the period.",
    searchFailed: "Failed to load transactions.",
    regionPlaceholder: "Search Region: (e.g.) Bundang-gu, Gangnam-gu",
    sourceNote: "",
    unitDeal: "Eok",
    unitCount: "records",
    unitPpy: "KRW 10k / pyeong",
    allTradeTypeNote: "Only sale transactions are available right now.",
    mapView: "Map View",
    listView: "List View",
    loadingMap: "Loading map...",
    mapLoadError: "Failed to load map. Please check your API key.",
    avgPriceLabel: "Avg",
    maxPriceLabel: "Max",
    tradeCount: "Deals",

    // Dashboard
    dashboardTitle: "Dashboard Summary",
    dashboardSubtitle: "Monitor real-time deals and alert triggers for your target areas.",
    activeRules: "Active Rules",
    totalMatches: "Accumulated Matches",
    sentNotifications: "Sent Alerts",
    systemStatus: "Status",
    statusNormal: "Active",
    statusCheck: "Check",
    alertHistory: "Alert Dispatch History",
    noAlertHistory: "No alert dispatch history.",
    alertSuccess: "Success",
    alertSkipped: "Skipped",
    alertFailed: "Failed",
    alertPrefix: "Alert",
    dbStatsTitle: "Local Real Estate DB",
    dbStatsRegionCount: "Collected Regions",
    dbStatsComplexCount: "Registered Complexes",
    dbStatsDealCount: "Accumulated Deals",
    dbStatsSubtitle: "MOLIT OpenAPI integration & local SQLite storage status",
    accordionCollapse: "Collapse",
    accordionExpand: "Expand",
    accordionHistoryCollapsed: "History is collapsed. Click to expand run results.",
    accordionAlertHistoryCollapsed: "Alert history is collapsed. Click to expand log.",

    // Rules
    rulesTitle: "Alert Rules",
    rulesSubtitle: "Create and manage rules to automatically track transactions in target regions.",
    ruleFormCreate: "Create Alert Rule",
    ruleFormEdit: "Edit Alert Rule",
    ruleName: "Rule Name",
    ruleNamePlaceholder: "e.g. Nearby Bargain Check",
    regionSearch: "Search Region",
    aptSelect: "Select Apartments",
    aptSelectDesc: "Select complexes to track (tracks all if none selected)",
    aptSearchPlaceholder: "Search and add complex names...",
    aptSearchInputPlaceholder: "Enter complex name (or leave empty for all)",
    priceRangeLabel: "Price Range (Eok)",
    priceMinPlaceholder: "Min",
    priceMaxPlaceholder: "Max",
    areaRangeLabel: "Area Range (㎡)",
    areaMinPlaceholder: "Min",
    areaMaxPlaceholder: "Max",
    comparisonCriteria: "Comparison Criteria",
    comparisonCriteriaNone: "Price Only",
    comparisonCriteriaParking: "Parking Space",
    comparisonCriteriaLargeComplex: "Large Complex",
    comparisonCriteriaTransit: "Transit Accessible",
    comparisonCriteriaNewer: "Newer / Build Year",
    comparisonCriteriaLivability: "Livability",
    ruleInitialName: "Track Target Region Deals",
    checkInterval: "Alert Time",
    checkIntervalNote: "Checks for new transaction alerts once a day at the designated time.",
    alertChannels: "Alert Channels",
    channelTelegram: "Telegram (Default)",
    channelKakao: "Kakao Send to Me (Reserved)",
    buttonPrev: "Back",
    buttonNext: "Next",
    buttonSave: "Save and Enable Rule",
    buttonSaveProgress: "Saving...",
    buttonSaveComplete: "Update Complete",
    ruleListTitle: "Alert Rules List",
    noRules: "No alert rules saved. Try adding a new rule.",
    ruleSuspended: "Paused",
    ruleAllComplexes: "All Complexes",
    ruleRecentTrack: "Auto tracks last 2 months",
    rulePriceRange: "Eok ~ ",
    ruleUnlimited: "Unlimited",
    ruleIntervalSuffix: "min interval",
    ruleLastChecked: "checked",
    ruleEdit: "Edit",
    ruleDelete: "Delete",
    rulePause: "Pause",
    ruleResume: "Resume",
    ruleRunNow: "Run Now",
    ruleRunProgress: "Running...",
    deleteConfirm: "Are you sure you want to delete this rule?",

    // Analytics
    analyticsTitle: "Real Estate Analytics",
    analyticsSubtitle: "Analyze time-series trends and multidimensional statistics from collected transaction data.",
    analyticsTabOverview: "Overview",
    analyticsTabComplex: "Complex Analysis",
    analyticsTabInsight: "AI Insight",
    analyticsTabNearby: "Nearby Station",
    totalTradeVolume: "Total Volume",
    averageTradePrice: "Average Price",
    highestTrade: "Highest Sale",
    lowestTrade: "Lowest Sale",
    nearbyRadiusLabel: "Search Radius",
    enableLiveLabel: "Live MOLIT API Search",
    liveSearchDesc: "Fetches live data from MOLIT API. It may take over 5 seconds and only displays complexes with transactions in the last 3 months.",
    nearbyRadius300m: "300m (5 min walk)",
    nearbyRadius500m: "500m (7 min walk)",
    nearbyRadius700m: "700m (10 min walk)",
    nearbyRadius1000m: "1km (15 min walk)",
    nearbyRadius1500m: "1.5km (20 min walk)",
    nearbyRadius2000m: "2km (30 min walk)",
    boxPlotTitleArea: "Deal Price Distribution & Volume by Size",
    boxPlotTitleFloor: "Deal Price Distribution & Volume by Floor",
    boxPlotTitleRegion: "Deal Price Distribution & Volume by Region",
    boxPlotTitleUnion: "Deal Price Distribution & Volume",
    dimensionArea: "by Size",
    dimensionFloor: "by Floor",
    pivotTableTitle: "Regional/Size-wise Real Estate Summary",
    pivotTableLegend: "* Hover over cells to view detailed real estate transaction information.",
    xaxisPyeong: "Size (Pyeong)",
    xaxisM2: "Size (㎡)",
    boxPlotWhisker: "Min-Max Range (Whisker)",
    boxPlotBox: "Q1-Q3 Distribution Box",
    boxPlotMedian: "Median",
    boxPlotAvg: "Average (Avg)",
    boxPlotVolume: "Volume (Right, Bg)",
    floorLow: "Low (5F or below)",
    floorMid: "Mid (6~15F)",
    floorHigh: "High (16~25F)",
    floorSuper: "Super High (26F+)",
    allRegions: "All Regions",
    complexActiveRankTitle: "Most Active Apartment Complexes",
    complexActiveRankTitleWithLimit: "Top {limit} Most Active Apartment Complexes",
    complexClickHint: "* Click the complex name (X-axis) or graph to view detailed transaction trends.",
    showLegend: "Show Legend",
    hideLegend: "Hide Legend",
    legendVolume: "Transaction Volume (Bar)",
    legendAvgPrice: "Average Price (Line)",
    noActiveComplexData: "No active transaction data in this region.",
    noActiveChartData: "No data to display. Please enable series in the legend.",
    axisVolume: "Volume (Count)",
    axisAvgPrice: "Avg Price (Eok)",

    // Collect Stats
    collectReportTitle: "Daily Collection Report",
    collectReportSubtitle: "Analyze the daily transaction volume collected and loaded into the DB by the collection bot.",
    dailyCollectChart: "Daily Collection Volume",
    collectCount: "Collection Count",
    selectDatePrompt: "Click a date on the graph to display the detailed collection count by region here.",
    selectedDateStats: "Collection Stats by Region",
    regionName: "Region",
    lawdCode: "Region Code",
    accumulatedCollectCount: "Accumulated Collections",
    totalAvgPrice: "Total Average Deal Price",
    maxComplexCount: "Max Complexes",
    unitComplex: "complexes",
    unitDealWon: "Eok KRW",
    periodRecent30: "Last 30 Days",
    periodAll: "All",
    dailyView: "Daily (by Collect Date)",
    monthlyView: "Monthly (by Deal Date)",
    monthlyCollectChart: "Collection Volume by Deal Month",
    selectMonthPrompt: "Click a deal month on the graph to display the detailed collection count by region here.",
    selectedMonthStats: "Collection Stats by Region for Month",
    dealMonth: "Deal Month",

    // Database Admin
    dbAdminTitle: "Database Admin",
    dbAdminSubtitle: "View SQLite schema, execute raw SQL queries directly, and inspect/modify database tables.",
    execute: "Execute",
    sqlPlaceholder: "Enter SQL query here. (e.g., SELECT * FROM transactions LIMIT 10;)",
    queryResult: "Query Result",
    tablesList: "Tables",
    schemaInfo: "Schema Columns",
    querySuccess: "Query executed successfully.",
    rowsCount: "rows",
    affectedRows: "Affected Rows",
    lastInsertId: "Last Insert ID",
    sqlTemplate: "SQL Templates",
    dbTabSql: "SQL Explorer",
    dbTabGeocoding: "Location & Coordinates",
    dbTabManagement: "Data Management",

    // Settings
    settingsTitle: "Settings",
    settingsSubtitle: "Manage data sources, external API keys, and real-time alert channel integrations.",
    systemStatusTitle: "System Integration Status",
    settingsSaved: "System settings successfully saved and applied.",
    settingsSaveFailed: "Failed to save settings.",

    // Allowed Accounts
    allowedAccounts: "Account Management",
    allowedAccountsTitle: "Account Management",
    allowedAccountsSubtitle: "Manage dashboard access accounts and temporary passwords.",
    allowedAccountsListTitle: "Registered Account List",
    addAccount: "Add Account",
    emailPlaceholder: "Enter email address (e.g. user@gmail.com)",
    deleteAccountConfirm: "Are you sure you want to remove this account from the list?",
    addSuccess: "Account added successfully.",
    deleteSuccess: "Account deleted successfully.",
    invalidEmail: "Invalid email format.",
    alreadyExists: "This email is already registered.",
    noAllowedAccounts: "No registered accounts found. Access may be restricted for all users.",
    role: "Role",
    userRoleNormal: "User",
    userRoleAdmin: "Admin",
    toggleAdminSuccess: "Role updated successfully.",
    cantRemoveSelfAdmin: "You cannot remove your own admin privileges.",
    atLeastOneAdmin: "At least one admin must exist.",
    resetPasswordTooltip: "Reissue Temporary Password",
    resetPasswordConfirm: "Are you sure you want to reset the password for {email} and reissue a temporary password?",
    resetPasswordSuccess: "Password successfully reset and reissued.",
    tempPasswordIssued: "Temporary password issued.",
    tempPasswordAlert: "A temporary password has been generated. Please copy it now as it cannot be shown again after closing this window.",
    copyTempPassword: "Copy Temp Password",
    copySuccess: "Copied to clipboard!",
    isAdminUser: "Grant Admin Privileges",
    changePasswordTitle: "Change Password Required",
    changePasswordSubtitle: "You logged in with a temporary password. Please change your password for secure access.",
    newPassword: "New Password",
    confirmNewPassword: "Confirm New Password",
    passwordMismatch: "Passwords do not match.",
    passwordMinLength: "Password must be at least 8 characters long.",
    changePasswordSuccess: "Password changed successfully. You can now use the dashboard.",
    changePasswordFailed: "Failed to change password.",

    // Nearby Station Search
    nearbyStationTitle: "Search Near Station",
    nearbyStationSubtitle: "Search apartment complexes and positions within a target radius from a subway station.",
    stationPlaceholder: "e.g. Pangyo Station, Gangnam Station",
    nearbyComplexesTitle: "Complexes within Radius",
    clickComplexToAnalyze: "Click a complex location to navigate to detailed analysis.",

    // Region Map Explorer
    regionMapTitle: "Regional Complex Map",
    regionMapSubtitle: "Explore apartment complexes, locations, and latest transaction prices on the map by loaded district.",
    regionMapNavLabel: "Complex Map",
    loadedRegions: "Loaded Districts",
    selectRegionToViewMap: "Select a district to explore on the map.",
    totalComplexesCount: "Total {count} complexes",
    geocodedComplexesCount: "Coordinates available: {count}",
    noComplexesInRegion: "No complexes loaded for the selected district.",
    searchComplexInRegion: "Search complex name or dong...",
    allDongs: "All Dongs",
    latestDealPrice: "Latest Price",
    avgDealPrice: "Average Price",
    dealCountLabel: "Deals",
    viewComplexAnalysisBtn: "Analyze Complex",
    addRuleForComplex: "Add Alert Rule",
    sortByTxCount: "Most Deals",
    sortByLatestDeal: "Latest Deal",
    sortByPriceHigh: "Highest Price",
    sortByPriceLow: "Lowest Price",
    sortByName: "Alphabetical",
    paginationInfo: "Showing {from}-{to} of {total}",
    pageItemsCount: "{count} per page",
    firstPageBtn: "First Page",
    prevPageBtn: "Previous Page",
    nextPageBtn: "Next Page",
    lastPageBtn: "Last Page",
    autoGeocodeNotice: "There are {count} complexes missing coordinates.",
    runBatchGeocodeForRegion: "Batch Fetch Coordinates",
    mapViewToggle: "Map View",
    listViewToggle: "List View",
    viewOnMap: "Map View",
    showOnMap: "Show on Map",
    centerToStation: "Center to Station",
    geocodeStatsBtn: "Coordinate Cache Settings",
    geocodeStatsTitle: "Geocoding Cache Administration",
    geocodePendingListTitle: "Complexes without Coordinates",
    noGeocodePendingComplexes: "All complexes have coordinates.",
    noNearbyComplexes: "No registered apartment complexes within this radius.",
    geocodePercentageOfTotal: "Coordinate Data Status",
    geocodeRequiredDesc: "Currently, there are no latitude/longitude coordinates for {pending} complexes among the apartments registered in the DB. To improve search speed, you can execute Kakao Geocoding batch collection based on land lot addresses.",
    runGeocodeBatchBtn: "Batch Collect Coordinates for Pending Complexes",
    batchGeocodeCompleted: "Geocoding Batch Collection Completed",
    batchGeocodeCountInfo: "Target: {total} / Success: {success} / Failed: {failed}",
    dbAggregateTab: "Pre-aggregated",
    liveSearchTab: "Real-time query",
    allComplexAlertRegister: "Register Alerts for All Complexes",
    alertRegisterTip: "Registers all complexes within the radius as interest notification rules.",
    complexAnalysisLink: "Go to Complex Analysis",
    alertRegisterBtn: "Register Alert Rule",
    stationSearchFailed: "Failed to query subway station.",
    geocodeBatchFailed: "Failed to execute Geocoding batch.",
    geocodeBatchPausing: "Pausing collection (Rate Limit Prevention)...",
    geocodeCollecting: "Collecting",
    distance: "Distance",
    telegramGuideTitle: "Telegram Setup Guide",
    telegramGuideSubtitle: "Guide to set up Telegram notifications when apartment transactions meet your conditions.",
    telegramStep1: "Step 1: Create a Bot (BotFather)",
    telegramStep1Desc: "Search for @BotFather in Telegram and send /newbot. Follow the instructions to choose a name and username for your bot. You will then receive the 'HTTP API Token'.",
    telegramStep2: "Step 2: Start Chat with the Bot & Send /start",
    telegramStep2Desc: "Click the bot link provided by BotFather to start a chat and send /start. This step is essential to permit the bot to send you messages.",
    telegramStep3: "Step 3: Get your Chat ID",
    telegramStep3Desc: "To get the destination Chat ID, search for @getidsbot in Telegram and start a chat, or add the bot to your group/channel where you want to receive alerts. (User ID is positive, Group/Channel ID is negative starting with -.)",
    telegramStep4: "Step 4: Save & Apply Settings",
    telegramStep4Desc: "Go to 'Settings > External API Settings > Telegram Channel' in the dashboard, input the Bot Token and Chat ID, then save the changes.",
    telegramOpenBotFather: "Go to @BotFather",
    telegramOpenGetIdsBot: "Go to @getidsbot",
    telegramCopy: "Copy",
    telegramCopied: "Copied!",
    telegramClose: "Close",
    activityLog: "Activity Log",
    activityLogTitle: "Activity Log Monitoring",
    activityLogSubtitle: "Monitor user activities including page views, transaction searches, and region updates.",
    activityType: "Activity Type",
    activityDescription: "Description",
    ipAddress: "IP Address",
    userAgent: "User Agent",
    time: "Time",
    filterUser: "Filter User Email",
    filterType: "Filter Activity Type",
    allTypes: "All Types",
    noActivityLogs: "No activity logs recorded.",
    activityTrends: "Recent Activity Trends (14 days)",
    activityVolume: "Activity Volume (Logs)",
    activeUsers: "Active Users (People)",
    activityDistribution: "Activity Distribution",
    payload: "Payload",
    viewPayload: "View Details",
    recentSearch: "Recent Search",
    recentSearchClear: "Clear All",
    recentRegions: "Recent Regions",
    recentStations: "Recent Stations",
    filterDate: "Filter Date",
    allDates: "All Dates",
    clearDateFilter: "Reset Date Filter",
    activityChartClickHint: "Click a date on the graph to filter logs for that specific day.",
    filteredByDate: "Activity logs for {date}",
    activityType_page_view: "Page View",
    activityType_search_transactions: "Search",
    activityType_region_add: "Add Region",
    activityType_region_delete: "Delete Region",
    activityType_rule_create: "Create Rule",
    activityType_rule_update: "Update Rule",
    activityType_rule_delete: "Delete Rule",
    activityType_rule_test: "Test Rule",
    activityType_preset_create: "Create Preset",
    activityType_preset_delete: "Delete Preset",

    // Infrastructure Rating
    infraRatingTitle: "⭐ Nearby Infrastructure Rating Report",
    infraRatingSubtitle: "Overall location score calculated based on weighted analysis of the shortest distance of key living infrastructures within a 1km radius.",
    infraTotalScore: "Overall Score",
    infraGrade: "Grade",
    infraCategory: "Category",
    infraScore: "Score",
    infraCount: "Count",
    infraMinDistance: "Min Distance",
    infraWalkMin: "Walk ~{min}m",
    infraMockNotice: "⚠️ Displaying sample infrastructure data because Kakao API key is not configured.",
    infraCategorySchool: "School",
    infraCategoryHospital: "Hospital",
    infraCategoryMart: "Mart",
    infraCategoryPharmacy: "Pharmacy",
    infraCategoryConvStore: "Conv. Store",
    infraDetailGeneralHospital: "General/Univ. Hospital",
    infraDetailLocalClinic: "Local Clinic",
    infraDetailLargeMart: "Large Mart",
    infraDetailSSM: "SSM Supermarket",
    infraDetailElementary: "Elementary School",
    infraDetailMiddle: "Middle School",
    infraDetailHigh: "High School",
    infraDetailMetro: "Subway (Metro)",
    infraDetailGtx: "GTX",
    infraDetailRail: "Rail/Train",
    infraDetailPharmacy: "Pharmacy",
    infraDetailConvStore: "Convenience Store",
    mapGuideTooltip: "Click or search a region on the map to manage transaction collection periods.",
    mapCollectPeriod: "Collection Period",
    mapTransactionCount: "Collected Transactions",
    mapCollectPromptTitle: "Transaction Collection Settings",
    mapCollectPromptNewRegion: "New Region Transaction Collection",
    mapCollectPromptExistingRegion: "Additional Transaction Collection (Extend Period)",
    mapCollectPromptConfirm: "Start Collection",
    mapCollectPromptCancel: "Cancel",
    mapCollectSuccess: "Transaction data successfully collected.",
    mapCollectFailed: "Failed to collect transaction data.",
    mapCollecting: "Collecting real transactions from MOLIT API...",
    collectProgressTitle: "Transaction Data Collection Progress",
    collectProgressStatus: "Collecting",
    collectingProgressText: "Collecting Transactions",
    stopBtn: "Stop",
    progressStatsTitle: "Progress Metrics",
    collectFailuresTitle: "Transaction Collection Failures",
    monthLabel: "Target Month",
    reasonLabel: "Failure Reason"
  }
} as const;

