...
| | **API 설명** | 지역코드와 기간을 설정하여 해당지역, 해당기간의 아파트 매매 실거래가 자료를 제공하는 아파트 매매 실거래가 정보 조회 서비스 |
...
| **상세기능 설명** | 행정표준코드관리시스템([www.code.go.kr](https://www.code.go.kr))의 법정동 코드 중 앞 5자리(예시 : 서울 종로구 - `11110`), 계약년월(예시 : `202407`)로 해당 지역, 해당 기간의 아파트 매매 신고정보를 조회 |
